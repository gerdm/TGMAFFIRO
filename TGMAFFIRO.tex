...
This type of process is also known as a fair-game. This is because what one would expect to win at any given future point in time is dependent on the amount that one has today. In other words, if both of the players start with a P\&L of \$0, then they would not expect, after a large amount of games\footnote{mathmatically speaking, the expectation of the game is zero in the sense that, as the number of games tends to infinity, the return of any of the players turn to zero}, to earn any more money that what they started with: \$0.
...
This simple process can be transformed into a process that takes $n$ steps between any two units of time. This new process is more alike to be the model of a financial asset that changes its value more than once between any two units of time.
