...
In what follows, we desire to express the dynamics of processes with respect to the Brownian motion. That is, we want to analyze how a process, perturbed by small changes of a brownian motion, behaves over time. The analysis of stochastic processes over time leads us to formulate what we would mean by an integral and its consequences, hence the name \textit{stochastic calculus}.\\

We begin by defining It\^o's integral, a crucial tool in analyzing a process \textit{perturbed} by a brownian motion; then, we extend the notion of an It\^o integral to another, more complete, space to work with; next, we prove Ito's formula, a noteworthy and necessary tool to further gain results; after, we comprehend results in the chapter and define what is called a stochastic differential equation, which allows us to express dynamics of processes in differential form and; we conclude the chapter showing Girsanov's theorem.
