...
\begin{proof}[proof of It\^o's formula]
	Consider $g_n \in C^2([0, \infty]\times\RNums)$ such that $g_n$ and its two first derivates are bounded for all $n$, and converge uniformly to $g$, $\partialwrt{g}{t}$, $\partialwrt{g}{x}$, and $\partialwrt[2]{g}{t}$. By \ref{prop:on_bounded_ito_g}, we can assume that such sequence of function exists.\\
	
	Taking $g(t, X(t))$ and expanding as a taylor series we see that:
	\begin{equation}
		g(t, X(t)) = g(0, X_0) + \sum_{i=0}^{n-1}\Delta g(t_i, X_i);
	\end{equation}
